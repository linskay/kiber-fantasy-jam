...
public class InputHandler {
    ...
    public void update(Vector2 velocity) {
        ...
        velocity.set(0, velocity.y); // Сохраняем вертикальную скорость (например, гравитацию)
        ...
        // Прыжок по W или пробелу, если игрок не прыгает
        if ((Gdx.input.isKeyJustPressed(Input.Keys.W) || Gdx.input.isKeyJustPressed(Input.Keys.SPACE)) && !player.isJumping()) {
            velocity.y = player.getJumpVelocity();
            player.setJumping(true);
        }
